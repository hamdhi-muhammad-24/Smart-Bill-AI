...
def build_lod_page1_pdf(data):
    ...
    # 5. Title (Centered at x=298.1, y=230.2, Calibri-Bold size 10)
    ...
    # 7. Company Name (x=62.2, y=297.7, Calibri-Bold 9)
    c.setFont("Calibri-Bold", 9)
    ...
    w_prefix = c.stringWidth("I write on the instructions of my Client Sri Lanka Telecom PLC, which has a Regional Office at ", "Calibri", 9)
    c.setFont("Calibri-Bold", 9)
    c.drawString(62.2 + w_prefix, y_top(317.6), reg_office)
    w_reg = c.stringWidth(reg_office, "Calibri-Bold", 9)
    c.setFont("Calibri", 9)
    c.drawString(62.2 + w_prefix + w_reg, y_top(317.6), " and its")

    draw_justified_line(c, "Head Office at Lotus Road, Colombo 01 and which is the Successor to all the assets, liabilities, rights, obligations and", "Calibri", 9, 62.2, y_top(329.6), max_w)
    ...
    # Paragraph 2 (y=361.1)
    draw_justified_line(c, "I am instructed that, you are a Customer of my Client and that, as such, at your request, my Client installed its", "Calibri", 9, 62.2, y_top(361.1), max_w)
    draw_justified_line(c, "telephone equipment and provided a telephone service to you at your premises bearing the above stated number,", "Calibri", 9, 62.2, y_top(372.4), max_w)
    draw_justified_line(c, "subject to the terms and conditions of the Agreement entered into by and between my client and you, including the", "Calibri", 9, 62.2, y_top(383.6), max_w)
    c.drawString(62.2, y_top(394.9), "payment of all subscriptions, charges, fees and other monies.")

    # Paragraph 3 (y=415.1)
    draw_justified_line(c, "I am instructed that, you have benefited from and used the said facilities and services provided by my client, but you", "Calibri", 9, 62.2, y_top(415.1), max_w)
    draw_justified_line(c, "have failed and neglected to pay the monies due as aforesaid, though my client has sent you Monthly Statements", "Calibri", 9, 62.2, y_top(426.4), max_w)
    ...
    w_p4_pre = c.stringWidth("I am instructed that, presently there is a sum of ", "Calibri", 9)
    c.setFont("Calibri-Bold", 9)
    c.drawString(62.2 + w_p4_pre, y_top(457.9), bal_str)
    w_p4_bal = c.stringWidth(bal_str, "Calibri-Bold", 9)
    c.setFont("Calibri", 9)
    c.drawString(62.2 + w_p4_pre + w_p4_bal, y_top(457.9), " owing from you to my Client, on account of the")

    draw_justified_line(c, "subscriptions, charges, fees and other monies due from you to my Client for the installation and provision of the said", "Calibri", 9, 62.2, y_top(469.9), max_w)
    c.drawString(62.2, y_top(481.1), "telephone services. You are liable and bound and obliged to pay these monies to my Client.")

    # Paragraph 5 (y=501.4)
    draw_justified_line(c, "However, you have wrongfully and unlawfully failed and neglected to pay these monies to my Client and the said", "Calibri", 9, 62.2, y_top(501.4), max_w)
    draw_justified_line(c, "monies payable by you to my Client, are in arrears and in default. Therefore, my Client has instructed me to advise", "Calibri", 9, 62.2, y_top(512.6), max_w)
    c.drawString(62.2, y_top(523.9), "that the aforesaid Agreement is hereby terminated and determined.")

    # Paragraph 6 (y=544.1)
    draw_justified_line(c, "I am also instructed to demand and I do hereby demand payment from you to my Client, of the aforesaid monies,", "Calibri", 9, 62.2, y_top(544.1), max_w)
    draw_justified_line(c, "within 14 days of the date of receipt of this letter and advise that if you fail to make such payment, legal action will be", "Calibri", 9, 62.2, y_top(555.4), max_w)

...
import { getOutputDates, getOutputCycles, getOutputBatches, getOutputPdfs, fetchPdfBlobUrl, downloadDateOutputZip } from '../../lib/api'
...
export default function OutputArchive() {
  ...
  const handleDownloadDateZip = async (dateStr: string, e?: React.MouseEvent) => {
    if (e) e.stopPropagation()
    if (downloadingDate) return
    setDownloadingDate(dateStr)
    const toastId = toast.loading(`Preparing ZIP for ${dateStr}...`)
    try {
      await downloadDateOutputZip(dateStr)
      toast.success(`Downloaded ${dateStr}.zip successfully`, { id: toastId })
    } catch (err: any) {
      toast.error(err?.message || `Failed to download ${dateStr} zip`, { id: toastId })
    } finally {
    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] gap-4">
      ...
      <div className="flex flex-1 min-h-0 gap-4 overflow-hidden">
        ...
        <div className="w-1/3 flex flex-col glass-card shadow-lg overflow-hidden h-full min-h-0">
          ...
          {/* Breadcrumb */}
          <div className="bg-muted/30 border-b p-3 flex flex-wrap gap-1 items-center justify-between text-sm shrink-0 min-h-[49px]">
            ...
            {selectedDate && /^\d{4}-\d{2}-\d{2}$/.test(selectedDate) && (
              <Button
                variant="outline"
                size="sm"
                className="h-7 px-2 text-xs gap-1.5 shrink-0 bg-background/50 hover:bg-muted"
                onClick={(e) => handleDownloadDateZip(selectedDate, e)}
                disabled={downloadingDate === selectedDate}
                title={`Download all outputs for ${selectedDate} as ZIP`}
              >
                {downloadingDate === selectedDate ? (
